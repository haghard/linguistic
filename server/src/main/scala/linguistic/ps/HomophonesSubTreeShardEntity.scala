...
class HomophonesSubTreeShardEntity(path: String)(implicit val mat: ActorMaterializer) extends PersistentActor
  ...
  override def preStart() = {
    ...
    log.info("Started Homophones Entity Actor for key [{}] from the file {}", entityKey, file.getAbsolutePath)
  }
  ...
  def indexing(index: SubTree): Receive = {
    ...
    case m: RestoredIndex[Array[String]]@unchecked =>
      if (m.index.count == 0) buildIndex(entityKey, path)
        ...
        unstashAll()
        log.info("Index has been recovered from snapshot with size {} for key [{}]", m.index.count, entityKey)
